...
uint8_t nRF24_Check(void)
{
	...
	nRF24_WriteMBReg(nRF24_CMD_W_REGISTER | nRF24_REG_RX_ADDR_P0, ptr, 5);
...
void nRF24_RX_ON(uint8_t pipe, uint8_t aa_state, uint8_t payload_len)
{
	nRF24_CE_OFF();
	/*Return to Standby-1*/
	nRF24_SetRXPipe(pipe, aa_state, payload_len);
	/*The pipe is set*/
	nRF24_SetOperationalMode(nRF24_MODE_RX);
	/*Set bit PRIM_RX = 1 */
	nRF24_CE_ON();
	/*Turn on the chip enable pin to start the Rx mode*/

}
void nRF24_TX_ON(void)
{
	nRF24_CE_OFF();
	/*Return to Standby-1*/
	...
	nRF24_CE_ON();
	HAL_Delay(1);
	/*Turn on the pin and wait to start the Tx Mode*/
}
void nRF24_SendPackage(uint8_t *data, uint8_t length)
{
